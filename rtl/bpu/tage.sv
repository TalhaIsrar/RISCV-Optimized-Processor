module tage_top #(
    ...
    logic [MAX_TAG-1:0] folded_tag [N_TABLES-1:0];
    logic [MAX_IDX-1:0] update_folded_idx [N_TABLES-1:0];
    logic [MAX_TAG-1:0] update_folded_tag [N_TABLES-1:0];
    ...
    generate
        for (i = 0; i < N_TABLES; i=i+1) begin : fold_tables
            ...
            fold_history #(
                ...
                .ghr(ghr_out),
                .folded_history(folded_tag[i][TAGE_TAG_SIZE[i]-1:0])
            );
            ...
            fold_history #(
                ...
                .ghr(ghr_ex),
                .folded_history(update_folded_tag[i][TAGE_TAG_SIZE[i]-1:0])
            );
    ...
    generate
        for (i = 0; i < N_TABLES; i=i+1) begin : tables
            tage_table #(.TAGE_IDX_SIZE(TAGE_IDX_SIZE[i]), .TAGE_TAG_SIZE(TAGE_TAG_SIZE[i])) t (
                ...
                .tag_i(pc_next_pred_i[TAGE_TAG_SIZE[i]+TAGE_IDX_SIZE[i]+1:TAGE_IDX_SIZE[i]+2] ^ folded_tag[i][TAGE_TAG_SIZE[i]-1:0]),
                .hit_o(tag_hits[i]),
                ...
                .update_tag_i(pc_ex[TAGE_TAG_SIZE[i]+TAGE_IDX_SIZE[i]+1:TAGE_IDX_SIZE[i]+2] ^ update_folded_tag[i][TAGE_TAG_SIZE[i]-1:0]),
