module btb_write #(parameter N = 32)(
    ...
    wire valid1;
    wire [29-$clog2(N):0] tag1;
    wire [31:0] target1;
    wire [1:0] state1;

    // Final write singals to put into BTB
    wire write_valid1;
    wire [29-$clog2(N):0] write_tag1;
    wire [31:0] write_target1;

    // Check for each branch in set
    wire entry_exists;

    wire [1:0] write_state1;
    ...
    assign valid1 = update_set[63];

    assign tag1 = update_set[62:33+$clog2(N)];               // 62:36, 62:37, 62:38
                                                    //Index: 3-1:0, 4-1:0, 5-1:0
    assign target1 = update_set[32+$clog2(N):1+$clog2(N)];   // 35:4,  36:5,  37:6
