module tage_top #(
    ...
    generate
        for (i = 0; i < N_TABLES; i=i+1) begin : tables
            tage_table #(.TAGE_IDX_SIZE(TAGE_IDX_SIZE), .TAGE_TAG_SIZE(TAGE_TAG_SIZE)) t (
                ...
                // IF-stage prediction
                .index_i(pc_pred_i[TAGE_IDX_SIZE+1:2] ^ folded_idx[i]),
                .tag_i(pc_pred_i[TAGE_TAG_SIZE+TAGE_IDX_SIZE+1:TAGE_IDX_SIZE+2] ^ folded_tag[i]),
                .hit_o(tag_hits[i]),
                ...
                .update_index_i(pc_upd_i[TAGE_IDX_SIZE+1:2] ^ update_folded_idx[i]),
                .update_tag_i(pc_upd_i[TAGE_TAG_SIZE+TAGE_IDX_SIZE+1:TAGE_IDX_SIZE+2] ^ update_folded_tag[i])
            );
        end
    endgenerate
    ...
    always_comb begin
        ...
        if (!alloc_found) begin
            alloc_table = $clog2(N_TABLES)'($urandom % N_TABLES);
        end
    end
